...
import { API_groupmanage } from '@/api/admin/dictionary/group';
        ...
        async registGroup(item) {
            ...
            await API_groupmanage(this.apiUID, param)
                .then((res) => {
                    ...
                    this.$store.dispatch('NOTY_COM', `기사 주제 등록을 성공했습니다.`);
                })
                .catch((err) => {
                    this.$store.dispatch('NOTY_ERR', `기사 주제 등록을 실패했습니다.`);
                })
                .finally(() => {
                    this.loading = false;
                });
        ...
        async deleteGroup(item) {
            ...
            await API_groupmanage(this.apiUID, param)
                .then(() => {
                    ...
                    this.$store.dispatch('NOTY_COM', `기사 주제 삭제를 성공했습니다.`);
                })
                .catch((err) => {
                    this.$store.dispatch('NOTY_ERR', `기사 주제 삭제를 실패했습니다.`);
                })
                .finally(() => {
                    this.loading = false;
                });
        ...
        async editGroup(item) {
            ...
            await API_groupmanage(this.apiUID, param)
                .then((res) => {
                    ...
                    this.$store.dispatch('NOTY_COM', `기사 주제 수정을 성공했습니다.`);
                })
                .catch((err) => {
                    this.$store.dispatch('NOTY_ERR', `기사 주제 수정을 실패했습니다.`);
                })
                .finally(() => {
                    this.loading = false;
                });
