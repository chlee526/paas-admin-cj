...
import { API_keywordlist } from '@/api/admin/classify/keyword';
import CompKeywordList from '@/components/keyword_boilerplate/comp_list';

export default {
    name: 'comp-classify-keyword-list',
    ...
    methods: {
        ...
        exportExcel() {
            ...
            worker.postMessage({ list: this.excelDownList, columns: setColnameHeader, sheetName: '관리자_기사_분류_사전' });
        ...
        async fetchData(excel) {
            ...
            const isSelected = ['seq', 'classify'].every((key) => Object.keys(this.selectedGrp).includes(key));
            if (!isSelected) {
